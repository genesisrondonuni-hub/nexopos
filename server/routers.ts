...
export const appRouter = router({
  ...
  salesAgent: router({
    respond: publicProcedure.input(z.object({
      ...
      fiscal: z.object({ countryCode: z.literal("VE"), currencyCode: z.literal("VES"), displayCurrency: z.enum(["VES", "USD"]), usdVesRate: z.number().min(0).max(100000000), usdVesRateUpdatedAt: z.string().datetime().nullable(), rif: z.string().max(32), ivaRate: z.number().min(0).max(31), documentMode: z.enum(["OPERATIVO", "DIGITAL_PENDIENTE"]) }),
