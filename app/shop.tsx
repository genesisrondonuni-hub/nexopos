...
import { colors, formatMoney } from "@/components/nexo-ui";
...
function CatalogItem({ product, accent }: { product: Product; accent: string }) {
...
export default function ShopScreen() {
  ...
  if (!configuration.features.catalog) return <ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-[#F6F3EE]" className="px-5"><View style={styles.catalogDisabled}><MaterialIcons name="storefront" size={34} color={colors.muted} /><Text style={styles.disabledTitle}>Catálogo no habilitado</Text><Text style={styles.emptyText}>Este perfil de {profile.label.toLowerCase()} opera sin catálogo público por ahora.</Text><Pressable onPress={() => router.back()} style={({ pressed }) => [styles.returnButton, pressed && styles.pressed]}><Text style={styles.returnText}>Volver</Text></Pressable></View></ScreenContainer>;
  return <ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-[#F6F3EE]" className="px-5"><FlatList data={catalogProducts} renderItem={({ item }) => <CatalogItem product={item} accent={experience.accent} />} keyExtractor={(item) => item.id} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} ListHeaderComponent={<><View style={styles.topbar}><Pressable onPress={() => router.canGoBack() ? router.back() : router.replace("/")} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><MaterialIcons name="arrow-back" size={21} color={colors.ink} /></Pressable><Text style={[styles.topbarLabel, { color: experience.accent }]}>{profile.shortLabel.toUpperCase()}</Text><Pressable onPress={() => router.push("/shop-checkout" as never)} style={({ pressed }) => [styles.cartButton, { backgroundColor: experience.accent }, pressed && styles.pressed]}><MaterialIcons name="shopping-bag" size={20} color={colors.white} />{itemCount ? <View style={styles.cartCount}><Text style={styles.cartCountText}>{itemCount}</Text></View> : null}</Pressable></View><View style={[styles.hero, { backgroundColor: experience.accent }]}><View style={[styles.shopLogo, { backgroundColor: experience.soft }]}><Text style={[styles.shopLogoText, { color: experience.accent }]}>{configuration.businessName.split(" ").map((word) => word[0]).join("").slice(0, 2).toUpperCase()}</Text></View><View><Text style={styles.shopName}>{configuration.businessName}</Text><View style={styles.openLine}><View style={styles.greenDot} /><Text style={styles.openText}>Abierto hasta las 9:00 p. m.</Text></View></View></View><View style={styles.intro}><Text style={styles.introTitle}>{profile.id === "RESTAURANT" || profile.id === "FAST_FOOD" ? "Nuestro menú" : "Nuestro catálogo"}</Text><Text style={styles.introBody}>{configuration.features.ageCheck ? "Compra responsable: confirmaremos que eres mayor de edad al entregar." : "Elige tus favoritos y envía el pedido directamente por WhatsApp."}</Text></View></>} ListFooterComponent={catalogProducts.length ? <View style={styles.footerSpace} /> : <View style={styles.empty}><MaterialIcons name="menu-book" size={32} color={colors.muted} /><Text style={styles.emptyText}>No hay productos disponibles en este momento.</Text></View>} /><View style={styles.checkoutBar}><View><Text style={styles.checkoutCaption}>{itemCount ? `${itemCount} productos seleccionados` : "Tu carrito está vacío"}</Text><Text style={styles.checkoutTotal}>{formatMoney(total)}</Text></View><Pressable disabled={!itemCount} onPress={() => router.push("/shop-checkout" as never)} style={({ pressed }) => [styles.checkoutButton, { backgroundColor: experience.accent }, !itemCount && styles.disabledButton, pressed && itemCount > 0 && styles.pressed]}><Text style={styles.checkoutButtonText}>Ver pedido</Text><MaterialIcons name="arrow-forward" size={18} color={colors.white} /></Pressable></View></ScreenContainer>;
}
...
const styles = StyleSheet.create({
  ...
  productCode: { fontSize: 9, fontWeight: "900", letterSpacing: 0.35, marginTop: 4 },
  ...
  catalogDisabled: { alignItems: "center", flex: 1, gap: 11, justifyContent: "center", paddingHorizontal: 34 },
  disabledTitle: { color: colors.ink, fontSize: 19, fontWeight: "800" },
  returnButton: { backgroundColor: colors.mint, borderRadius: 11, marginTop: 5, paddingHorizontal: 16, paddingVertical: 11 },
