...
export default function CrmScreen() {
  ...
  const renderOpportunity = ({ item }: { item: SalesOpportunity }) => {
    ...
    return <Card style={styles.opportunityCard}>
      ...
      {item.subject ? <View style={styles.subjectRow}><MaterialIcons name={item.appointmentAt ? "event-available" : "receipt-long"} size={15} color={colors.green} /><Text numberOfLines={2} style={styles.subjectText}>{item.subject}{item.appointmentAt ? ` · ${item.appointmentAt}` : ""}{item.address ? ` · ${item.address}` : ""}</Text></View> : null}
      <View style={styles.opportunityMiddle}><View style={styles.phone}><MaterialIcons name="chat" size={15} color={colors.green} /><Text style={styles.phoneText}>{item.phone}</Text></View><Text style={styles.value}>{formatMoney(item.value)}</Text></View>
...
const styles = StyleSheet.create({
  ...
  agendaButton: { alignItems: "center", backgroundColor: colors.white, borderColor: "#B5E0CF", borderRadius: 14, borderWidth: 1, height: 44, justifyContent: "center", width: 44 },
  ...
  subjectRow: { alignItems: "flex-start", backgroundColor: "#F8F8F6", borderRadius: 10, flexDirection: "row", gap: 7, padding: 9 },
  subjectText: { color: colors.ink, flex: 1, fontSize: 10, fontWeight: "700", lineHeight: 14 },
