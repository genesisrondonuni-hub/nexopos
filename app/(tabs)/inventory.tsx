...
export default function InventoryScreen() {
  ...
  const renderProduct = ({ item }: { item: Product }) => {
    ...
    return <Card style={styles.productCard}>
      <Pressable onPress={() => router.push({ pathname: "/product-detail", params: { productId: item.id } } as never)} style={({ pressed }) => [styles.productIcon, { backgroundColor: atMinimum ? "#FDE9E4" : colors.mint }, pressed && styles.pressed]}>{item.imageUri ? <Image source={{ uri: item.imageUri }} style={styles.productPhoto} /> : <MaterialIcons name={item.type === "SERVICE" ? "room-service" : "inventory-2"} size={20} color={atMinimum ? colors.coral : colors.green} />}</Pressable>
...
const styles = StyleSheet.create({
  ...
  imageImportButton: { alignItems: "center", backgroundColor: colors.mint, borderRadius: 14, height: 44, justifyContent: "center", width: 44 },
