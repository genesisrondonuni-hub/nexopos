...
export default function OrdersScreen() {
  const { orders, updateOrderStatus } = useNexo();
  const renderOrder = ({ item }: { item: Order }) => <Card style={styles.orderCard}>
    ...
    <View style={styles.orderBottom}><Text style={styles.total}>{formatCOP(item.total)}</Text><Pressable disabled={item.status === "ARCHIVADO"} onPress={() => { haptic.medium(); updateOrderStatus(item.id, nextStatus(item.status)); }} style={({ pressed }) => [styles.statusAction, item.status === "ARCHIVADO" && styles.disabledAction, pressed && item.status !== "ARCHIVADO" && styles.pressed]}><Text style={styles.statusActionText}>{actionLabel(item.status)}</Text><MaterialIcons name="arrow-forward" size={15} color={colors.green} /></Pressable></View>
  </Card>;
