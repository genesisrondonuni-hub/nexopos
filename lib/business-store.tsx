...
import { DEFAULT_VENEZUELAN_FISCAL_SETTINGS, normalizeIvaRate, normalizeRateHistory, normalizeVenezuelanRif, recordManualUsdVesRate } from "../shared/venezuela-fiscal";
...
export function BusinessProvider({ children }: { children: ReactNode }) {
    ...
    const restore = async () => {
      try {
        ...
        if (savedProfile && parsed.businessName) {
          ...
          setConfiguration({ ...parsed, suggestedCategories: savedProfile.suggestedCategories, categories: [...new Set(normalizedCategories)], features: { ...savedProfile.features, ...parsed.features }, copy: { ...getProfileCopy(savedProfile.id), ...parsed.copy }, fiscal: { ...DEFAULT_VENEZUELAN_FISCAL_SETTINGS, ...parsed.fiscal, countryCode: "VE", currencyCode: "VES", displayCurrency: parsed.fiscal?.displayCurrency === "USD" ? "USD" : "VES", usdVesRate: normalizeUsdVesRate(parsed.fiscal?.usdVesRate ?? 0), usdVesRateUpdatedAt: typeof parsed.fiscal?.usdVesRateUpdatedAt === "string" ? parsed.fiscal.usdVesRateUpdatedAt : null, usdVesRateSource: typeof parsed.fiscal?.usdVesRateSource === "string" ? parsed.fiscal.usdVesRateSource.slice(0, 160) : "", usdVesRateHistory: normalizeRateHistory(parsed.fiscal?.usdVesRateHistory), provider: { ...DEFAULT_VENEZUELAN_FISCAL_SETTINGS.provider, ...parsed.fiscal?.provider, verificationStatus: parsed.fiscal?.provider?.verificationStatus === "VERIFICADO" ? "VERIFICADO" : parsed.fiscal?.provider?.verificationStatus === "PENDIENTE" ? "PENDIENTE" : "NO_CONFIGURADO" }, rif: normalizeVenezuelanRif(parsed.fiscal?.rif ?? ""), ivaRate: normalizeIvaRate(parsed.fiscal?.ivaRate ?? DEFAULT_VENEZUELAN_FISCAL_SETTINGS.ivaRate) } });
        }
      } catch {
  ...
  const updateFiscal = useCallback((changes: Partial<BusinessConfiguration["fiscal"]>) => {
    setConfiguration((current) => { const rate = changes.usdVesRate === undefined ? current.fiscal.usdVesRate : normalizeUsdVesRate(changes.usdVesRate); const rateChanged = changes.usdVesRate !== undefined && rate !== current.fiscal.usdVesRate; const recordedAt = rateChanged ? new Date().toISOString() : current.fiscal.usdVesRateUpdatedAt; const source = changes.usdVesRateSource === undefined ? current.fiscal.usdVesRateSource : changes.usdVesRateSource.trim().slice(0, 160); return { ...current, fiscal: { ...current.fiscal, ...changes, countryCode: "VE", currencyCode: "VES", displayCurrency: changes.displayCurrency === "USD" ? "USD" : changes.displayCurrency === "VES" ? "VES" : current.fiscal.displayCurrency, usdVesRate: rate, usdVesRateUpdatedAt: recordedAt, usdVesRateSource: source, usdVesRateHistory: rateChanged ? recordManualUsdVesRate(current.fiscal.usdVesRateHistory, rate, source, recordedAt ?? undefined) : current.fiscal.usdVesRateHistory, rif: changes.rif === undefined ? current.fiscal.rif : normalizeVenezuelanRif(changes.rif), ivaRate: changes.ivaRate === undefined ? current.fiscal.ivaRate : normalizeIvaRate(changes.ivaRate) } }; });
  }, []);
