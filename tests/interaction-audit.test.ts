...
describe("auditoría de interacciones", () => {
  ...
  it("conserva pantallas reales para los accesos administrativos y de soporte", () => {
    ["app/team-permissions.tsx", "app/billing.tsx", "app/help-center.tsx", "app/sales-analytics.tsx", "app/business-profile.tsx"].forEach((path) => expect(existsSync(resolve(root, path))).toBe(true));
    const layout = readFileSync(resolve(root, "app/_layout.tsx"), "utf8");
    ["team-permissions", "billing", "help-center"].forEach((route) => expect(layout).toContain(`name=\"${route}\"`));
  });
});
