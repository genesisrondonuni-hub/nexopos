import { getProfileStarterCatalog } from "./business-profile-content";
...
const CONTEXT: Record<BusinessProfileId, DemoContext> = {
  ...
  MEDICAL_OFFICE: { customer: "Laura Gómez", phone: "3005550183", subject: "Cita de consulta general · 09:30", appointmentAt: "Mañana · 09:30", location: "Sede principal", delivery: "Recogida", source: "CATÁLOGO", crmSource: "CATÁLOGO" },
  CLINICAL_LAB: { customer: "Valeria Torres", phone: "3012124208", subject: "Cita para toma de muestra · Hemograma", appointmentAt: "Mañana · 08:00", location: "Sede de toma de muestras", delivery: "Recogida", source: "CATÁLOGO", crmSource: "CATÁLOGO" },
  DENTAL_CLINIC: { customer: "Sofía Martínez", phone: "3158816712", subject: "Valoración odontológica · 15:00", appointmentAt: "Viernes · 15:00", location: "Sede odontológica", delivery: "Recogida", source: "CATÁLOGO", crmSource: "WHATSAPP" },
  VETERINARY_LAB: { customer: "Clínica Patas", phone: "3174049120", subject: "Recepción de muestra veterinaria · 10:00", appointmentAt: "Mañana · 10:00", location: "Recepción de muestras", delivery: "Recogida", source: "POS", crmSource: "PRESENCIAL" },
  VETERINARY_OFFICE: { customer: "Mariana López", phone: "3017406505", subject: "Cita veterinaria para mascota · 11:30", appointmentAt: "Sábado · 11:30", location: "Consultorio veterinario", delivery: "Recogida", source: "CATÁLOGO", crmSource: "WHATSAPP" },
...
export function getProfileDemoData(profileId: BusinessProfileId, now = Date.now()) {
  ...
  const products: Product[] = getProfileStarterCatalog(profileId).map((item, index) => ({ id: `demo-${profileId.toLowerCase()}-${index + 1}`, code: createProductCode(`${profileId}-${item.name}`, index), name: item.name, description: item.description, category: item.category, collection: item.collection, colors: item.colors, sizes: item.sizes, price: item.price ?? 0, cost: item.cost ?? 0, stock: item.stock ?? (item.type === "SERVICE" ? 1 : 12), minStock: 1, showInCatalog: true, type: item.type }));
...
export function getProfileDemoOpportunities(profileId: BusinessProfileId): SalesOpportunity[] {
  ...
  return [
    ...
    { id: `demo-crm-${profileId.toLowerCase()}-2`, customerName: profileId === "RESTAURANT" ? "Mesa 03" : "Cliente de ejemplo", phone: "3025558190", stageId: "contacted", source: "WHATSAPP", value: catalog[1]?.price ?? catalog[0]?.price ?? 0, lastActivity: "Hace 26 min", subject: profileId === "CLINICAL_LAB" ? "Agenda de perfil lipídico · 10:30" : `Seguimiento · ${catalog[1]?.name ?? "Solicitud comercial"}`, appointmentAt: context.appointmentAt, address: context.location },
  ];
}
